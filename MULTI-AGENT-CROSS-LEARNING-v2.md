...
└────────────────────────────────────────────────────────────────────┘
         ...
         │ 各 Agent 调用 message(action="send", to="oc_xxx", channel="feishu")
         │
┌────────────────────────────────────────────────────────────────────┐
...
message(
  action: "send",
  to: "oc_xxx",        // 飞书群 chat_id
  channel: "feishu",    // 指定飞书频道
  message: "消息内容"   // 文本内容
)
```

> **技术验证结论**：没有飞书频道上下文的 session（如 cron isolated session 或 sessions_send 唤起的 session）可以自由指定 `to` + `channel` 参数发消息，不受 cross-context 限制。这是因为 cross-context 检查仅在 session 已绑定频道时才生效，无绑定 session 直接跳过检查。
...
3. sessions_send → PM: "请用 message 工具        ↑ PM 调用 message(action="send",
   在飞书群分享总结"                               to="oc_xxx", channel="feishu")
...
| 架构师 | PM、后端、前端 | 技术选型受需求驱动，架构规范约束实现 |
| UI 设计 | PM、前端 | 设计基于需求，落地依赖前端 |
| 前端 | UI 设计、后端 | 实现依赖设计稿和 API |
| 后端 | 架构师、PM | 实现遵循架构规范，API 服务产品需求 |
| 测试 | PM、后端、前端 | 测试覆盖需求和实现 |
...
```
请在飞书群中以你的身份分享以下总结。
使用 message 工具：action="send", to="oc_xxx", channel="feishu"
...
### 第二步：展示总结
用 message 工具在飞书群发开场消息（action="send", to="oc_xxx", channel="feishu"）。
然后对每个已收集总结的角色，发一条 sessions_send：
"请用 message 工具在飞书群分享你的总结：action='send', to='oc_xxx', channel='feishu'"
附上整理后的总结内容。
...
1. 使用 message 工具：action="send", to="oc_xxx", channel="feishu"
2. 消息中用 @ 标注你提到的其他角色
...
**配置要点**：
- `tools.agentToAgent.enabled` + `allow`：控制哪些 Agent 可以发起 A2A 通信（开关+白名单）
- `session.agentToAgent.maxPingPongTurns: 0`：禁用自动乒乓，本方案使用 Coordinator 手动编排
...
| session 中途崩溃 | 已收集数据丢失。Bitable 中增加 review_sessions 表记录当前阶段，用于崩溃恢复 |
...
**验证标准**：
- [ ] 4 个飞书机器人在群里正常发消息
- [ ] sessions_send 唤起的 Agent 能通过 message(action="send", to="oc_xxx", channel="feishu") 发消息到群
